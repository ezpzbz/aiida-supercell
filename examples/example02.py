...
def example_02(code: orm.Code):
    ...
    structure = orm.StructureData(pymatgen_structure=strc)
    ...
    builder.save_as_archive = orm.Bool(False)
    builder.metadata.options.max_wallclock_seconds = 1 * 30 * 60

    builder.sample_structures = orm.Dict(
        dict={
            'low_energy': 2,
            'high_energy': 2,
            'random': 2,
